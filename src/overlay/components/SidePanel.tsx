...
export const SidePanel: React.FC = () => {
  ...
  return (
    <div
      className={`fixed right-0 top-0 h-full w-[460px] bg-[#0B0D12] text-[#E5E7EB] border-l border-[#242936] flex flex-col z-[2147483645] pointer-events-auto ${
    ...
    >
      {/* Header */}
      <div className="h-[52px] px-4 border-b border-[#242936] bg-[#0B0D12] flex items-center justify-between flex-shrink-0">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-[14px] font-semibold text-[#F3F4F6] tracking-tight">PixelBrief List</h2>
          <span className="bg-[#1F2430] text-[#AEB6C2] text-[10px] font-semibold px-2 py-0.5 rounded-[4px] font-sans">
            {annotations.length}
          </span>
        </div>
        <button
          onClick={() => setSidebarOpen(false)}
          className="p-1.5 hover:bg-[#171B24] text-[#7D8594] hover:text-[#E5E7EB] rounded-md transition-all duration-150"
        >
          <X size={16} />
        </button>
      </div>
      ...
      {/* List content */}
      <div className="flex-1 overflow-y-auto p-[14px] space-y-[12px] bg-[#0B0D12]">
        {annotations.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center py-20 px-6 text-center select-none bg-[#0B0D12]">
            <div className="w-10 h-10 rounded-lg border border-[#252B38] flex items-center justify-center mb-4">
            ...
            <p className="text-[15px] font-semibold text-[#D1D5DB]">No annotations yet</p>
            <p className="text-[13px] text-[#8C94A3] max-w-[300px] leading-relaxed mt-1.5">
            ...
            .map((ann) => {
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-[8px] border text-left transition-all duration-150 cursor-pointer ${
                    isSelected 
                      ? 'bg-[#141A26] border-[#3B82F6]/50' 
                      : 'bg-[#121620] border-[#252B38] hover:border-[#343B4C]'
                  }`}
                  style={isSelected ? { boxShadow: '0 0 0 1px rgba(59,130,246,0.08)' } : undefined}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      {/* Color swatch dot */}
                      <span
                        className="w-[6px] h-[6px] rounded-full border border-zinc-950 ring-1 ring-zinc-800"
                    ...
                    <div className="flex items-center space-x-1">
                      <button
                        ...
                        className="p-1 hover:bg-[#171B24] text-[#7D8594] hover:text-[#E5E7EB] rounded transition-colors"
                      >
                        <Focus size={13} />
                      </button>
                      <button
                        ...
                        className="p-1 hover:bg-[#171B24] text-[#7D8594] hover:text-[#EF4444] rounded transition-colors"
                      >
                        <Trash2 size={13} />
                      </button>
                    </div>
                  ...
                  {/* Comment Editor */}
                  {ann.type === 'text' && (
                    <div className="text-xs text-[#E5E7EB] font-medium px-[10px] py-[9px] bg-[#0D1017] rounded-[6px] border border-[#2A3040] select-all font-mono break-all leading-relaxed mb-2">
                      {ann.text}
                    </div>
                  )}
                  <textarea
                    ...
                    className="w-full bg-[#0D1017] border border-[#2A3040] rounded-[6px] text-[#E5E7EB] placeholder-[#6B7280] py-[9px] px-[10px] min-h-[44px] text-[12px] transition-all duration-150 resize-none outline-none focus:border-[#3B82F6]/50 focus:shadow-[0_0_0_2px_rgba(59,130,246,0.08)] leading-relaxed"
                  />
                  
                  <div className="text-[11px] text-[#6B7280] mt-[10px] flex justify-between items-center font-sans">
                    <span>
      ...
      {/* Footer / Actions */}
      <div className="p-4 border-t border-[#242936] bg-[#0B0D12] space-y-[10px] flex-shrink-0">
        {/* Copy AI Prompt CTA */}
        <button
          ...
          className={`w-full h-[38px] flex items-center justify-center space-x-2 rounded-[6px] text-xs font-semibold text-white transition-all duration-150 active:scale-[0.98] border-none ${
            annotations.length === 0
              ? 'bg-[#1F2430] text-[#AEB6C2]/40 cursor-not-allowed'
              : copied
        ...
        <button
          ...
          className={`w-full h-[36px] flex items-center justify-center space-x-2 rounded-[6px] text-xs font-semibold border transition-all duration-150 active:scale-[0.98] ${
            annotations.length === 0 || isExporting
              ? 'bg-[#121620] border-[#252B38]/60 text-[#AEB6C2]/40 cursor-not-allowed'
              : 'bg-[#121620] border-[#2A3040] hover:bg-[#171B24] hover:border-[#3A4254] text-[#D1D5DB] cursor-pointer'
          }`}
        >
          {isExporting ? (
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] font-semibold">
          <button
            ...
            disabled={annotations.length === 0}
            className="flex items-center justify-center space-x-1.5 h-[32px] bg-[#10141D] border border-[#252B38] text-[#AEB6C2] hover:bg-[#171B24] rounded-[5px] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <Download size={11} />
            <span>prompt.md</span>
          </button>
          ...
          <button
            ...
            className="flex items-center justify-center space-x-1.5 h-[32px] bg-[#10141D] border border-[#252B38] text-[#AEB6C2] hover:bg-[#171B24] rounded-[5px] transition-colors col-span-2"
          >
            <Upload size={11} />
        ...
        {annotations.length > 0 && (
          <button
            onClick={clearAll}
            className="w-full text-center text-[10px] uppercase font-bold tracking-wider text-[#F87171] hover:text-[#EF4444] py-2 transition-all duration-150"
          >
